...
Description: Creating a linked list class with head and tail pointers
Comments: All tests must be used one test at a time.
*/
...
class ListT
{
	...
	/*
	pre: None. This is the constructor function, invoked like this from the the client program: 				List1 lst;
	or
	List1* lst = new List1;
	...
	*/ 
	...
	/*
	pre: An instance of ListT exists.  This is the destructor function, automatically invoked in the 			case of a static declaration, explcitly invoked, using the delete key word, in the case of a 				dynamic declaration.
	...
	*/	
	...
	/*	
	pre:  an instance of ListT exists and is not empty
	post: deletes all nodes that store target.  Returns the number of nodes deleted . 
	*/
	int DeleteItem(const itemType target);
	
	/*	
	pre:  an instance of ListT exists and is not empty
	post: deletes all nodes that store target.  Returns true if node is deleted, false otherwise. 
	*/
	bool DeleteAnItem(itemType target);
